...
func NewLocalStorage(conf Config) (localStorage LocalStorage, err error) {
...
func (o LocalStorage) CheckSignedURL(signedURL string, method string, expireAt int64, signature string) (err error) {
	...
	fileURL.RawQuery = fmt.Sprintf("expire_at=%d", expireAt)
...
func (o LocalStorage) SignURL(objectKey string, method utils.HTTPMethod, expiredInSec int64) (signedURL string, err error) {
	...
	fileURL, err := url.Parse("your file url")
	if err != nil {
	...
	fileURL.RawQuery = fmt.Sprintf("expire_at=%d", time.Now().Unix()+expiredInSec)

	signData := fmt.Sprintf("path=%s;method=%s;query=%s;sign_key=%s", fileURL.Path, method, fileURL.RawQuery, "your sign_key")
	log.Debugf("signData,%s", signData)
...
func (o LocalStorage) Delete(objectKeys ...string) (deletedObjects []string, err error) {
	...
	for _, key := range objectKeys {
		filePath, fErr := o.AbsPath(key)
		if fErr != nil {
			log.Errorf("AbsPath failed ,err is %v", fErr)
			continue
		}

		fErr = os.Remove(filePath)
		if fErr != nil {
			log.Errorf("os.Remove failed", fErr)
			continue
		}
