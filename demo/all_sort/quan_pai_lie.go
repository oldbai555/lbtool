...
func insert(res [][]string, insertNum string) (result [][]string) {
	// 保存结果的slice
	...
	for _, v := range res {
		for i := 0; i < len(v); i++ {
			// 在v的每一个元素前面插入
			var strArr []string
			strArr = append(strArr, v[:i]...)
			strArr = append(strArr, insertNum)
			strArr = append(strArr, v[i:]...)
			result[index] = append(result[index], strArr...)
			index++
		}

		// 在v最后面插入
		result[index] = append(result[index], append(v, insertNum)...)
		index++
	}
