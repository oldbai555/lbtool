...
// appendFileWithBuffer 自定义
func (m *Message) appendFileWithBuffer(list []*file, name string, buf []byte, settings []FileSetting) []*file {
	f := &file{
		Name:   filepath.Base(name),
		Header: make(map[string][]string),
		CopyFunc: func(w io.Writer) error {
...
// AttachBuffer 自定义
func (m *Message) AttachBuffer(filename string, buf []byte, settings ...FileSetting) {
	m.attachments = m.appendFileWithBuffer(m.attachments, filename, buf, settings)
}
