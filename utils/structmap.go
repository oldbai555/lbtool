...
func StructToMapV2(s interface{}) (res map[string]interface{}, err error) {
	v := reflect.ValueOf(s)
	...
	for i := 0; i < t.NumField(); i++ {
		...
		// read tag
		tagVal, flag := readTag(fieldType, tagJson)
		...
		case reflect.Slice, reflect.Array:
			...
			for i := 0; i < fieldValue.Len(); i++ {
				...
				deepRes, deepErr := StructToMapV2(elem.Interface())
				if deepErr != nil {
					return nil, deepErr
				}
		...
		case reflect.Struct:
			// recursive
			deepRes, deepErr := StructToMapV2(fieldValue.Interface())
			if deepErr != nil {
				return nil, deepErr
			}
			res[tagVal] = deepRes
		case reflect.Map:
			...
			for iter.Next() {
				newRes[fmt.Sprintf("%v", iter.Key().Interface())], _ = StructToMapV2(iter.Value().Interface())
			}
			res[tagVal] = newRes
		case reflect.Chan:
			res[tagVal] = fieldValue
